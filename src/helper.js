  ...
  .then((dbMonthObj) => {
    if (dbMonthObj !== null) {
      ...
      if (missingDays.length !== 0) { // > 0 means some days are missing. Go and get them from entsoe ...
        console.log(`helper.validateAndRepairMonthObject(): Found missing days for ${montName}, ${year}. missing days: `, missingDays);
...
const getLatestDailyElSpotPrices = async () => {
  ...
  if (! await validateAndRepairMonthObject(spotPrices.year, spotPrices.monthName, highestDayNr)) {
    console.log("Inconsistent data in ",spotPrices.monthName, spotPrices.year);
    return ("Inconsistent data in DB")
  }

  todayDateUTC = convertDateToUTC(new Date());
  console.log("getting today: ", new Date(), " UTC: ", todayDateUTC);
        ...
        .then( async () => {
          ...
          .then(async function (dbRsp) {
            if (dbRsp != null && dbRsp.monthName === spotPrices.monthName) {
              ...
              .then((acknowledged) => {
                if (!acknowledged) {
                  console.log("ERROR: FAILED to update DB eith todays el-spot prices");
                  return "FAILED to update DB eith todays el-spot prices";
                }
                console.log("DB updated with insertion of todays elspot prices");
              ...
              .then(async function (entsoeRspMonth) {
                ...
                .then( async () => {
                  ...
                  .then((acknowledged_1) => {
                    ...
                    console.log("DB updated with creation of a new month object, containing says until today.");
...
module.exports = { monthsAsTextList, addCustomCurrency, areDayObjectsInArraySorted, sortDayObjectsInArray, validateAndRepairMonthObject, getLatestDailyElSpotPrices, convertDateToUTC };
